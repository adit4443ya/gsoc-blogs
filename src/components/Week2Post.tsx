    ...
    <div className="post-content space-y-8">
      ...
      <div className="relative">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
          Following the foundation laid in Week 1, where I proposed the <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> ASR node to address the limitations of LFortran’s existing OpenMP support, Week 2 focused on implementing this design. In my previous blog post, I outlined a plan to represent the <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> node in the Abstract Semantic Representation (ASR) for the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct and validate minimal reproducible examples (MREs) against GFortran and Clang outputs. This week, I successfully implemented the foundational structure for the <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> node through PR <a href="https://github.com/lfortran/lfortran/pull/7449" className="text-indigo-500 dark:text-indigo-400 hover:underline">#7449</a>, dedicating approximately 20 hours to this task.
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
          I chose to implement the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct first for several strategic reasons. The <span className="font-semibold text-indigo-600 dark:text-indigo-400">parallel do</span> and <span className="font-semibold text-indigo-600 dark:text-indigo-400">parallel sections</span> constructs represent fundamentally different paradigms: the former is loop-based, while the latter is non-loop-based, allowing independent code blocks to execute concurrently. The existing design in LFortran was heavily adapted to the <span className="font-semibold text-indigo-600 dark:text-indigo-400">parallel do</span> construct, directly mapping it to a <span className="font-semibold text-indigo-600 dark:text-indigo-400">DoConcurrentLoop</span> node in the ASR. Shifting <span className="font-semibold text-indigo-600 dark:text-indigo-400">parallel do</span> to the new <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> design would have required significant changes to the OpenMP pass, which requires longer PRs and possibly regression (which means the testcases which we compile now of parallel do may not be compiled till openmp pass gets matured enough to process OMPRegion), which is not my intention to do for now.
        </p>
      ...
      <div>
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
          The previous design in LFortran was tailored specifically for the <span className="font-semibold text-indigo-600 dark:text-indigo-400">parallel do</span> construct, which limited its flexibility. When a <span className="font-semibold text-indigo-600 dark:text-indigo-400">parallel do</span> directive was encountered, the <span className="font-semibold text-indigo-600 dark:text-indigo-400">visit_Pragma</span> function assumed the immediate presence of a do loop, collecting its body through a <span className="font-semibold text-indigo-600 dark:text-indigo-400">visit_DoLoop</span> and converting it into a <span className="font-semibold text-indigo-600 dark:text-indigo-400">DoConcurrentLoop</span> node in the ASR. This approach was restrictive because OpenMP constructs like <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> or <span className="font-semibold text-indigo-600 dark:text-indigo-400">task</span> can contain arbitrary statements or nested directives, which do not fit the loop-centric model.
        </p>
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed mt-4">
          In my initial attempt to implement the <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> node, I focused on a special case for the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct, aiming to represent its structure in the ASR. However, during discussions with my mentor Pranav Goswami in our weekly calls, we recognized that this approach lacked the generality needed to support a wider range of OpenMP constructs, especially those with nested structures.
        </p>
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed mt-4">
          Following this feedback, I revised the implementation to adopt a stack-based approach for collecting the body of an <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> from <span className="font-semibold text-indigo-600 dark:text-indigo-400">transform_stmts</span> visitor which can be used to collect any type of stmts.The stack-based approach ensures that the implementation is generic and extensible, capable of representing any OpenMP construct, including nested structures, in a robust manner. This design has been implemented in PR <a href="https://github.com/lfortran/lfortran/pull/7449" className="text-indigo-500 dark:text-indigo-400 hover:underline">#7449</a>, providing a solid foundation for further OpenMP enhancements in LFortran.
        </p>
      </div>
      ...
      <div>
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
          To validate the stack-based approach and the new <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> implementation, I developed three MREs, focusing on the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> and <span className="font-semibold text-indigo-600 dark:text-indigo-400">task</span> constructs. These examples demonstrate the ability of the new design to handle both simple and nested OpenMP directives, accurately representing them in the ASR.
        </p>
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
          The first MRE illustrates a <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct with a <span className="font-semibold text-indigo-600 dark:text-indigo-400">reduction</span> clause, distributing independent tasks across multiple threads:
        </p>
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed mt-4">
          The corresponding ASR representation captures the nested structure of the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct, including the <span className="font-semibold text-indigo-600 dark:text-indigo-400">reduction</span> clause and individual <span className="font-semibold text-indigo-600 dark:text-indigo-400">section</span> regions:
        </p>
        <details className="mt-4">
          <summary className="cursor-pointer text-lg font-medium text-indigo-600 dark:text-indigo-400 hover:underline">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed">
          The second MRE demonstrates a more complex scenario involving a <span className="font-semibold text-indigo-600 dark:text-indigo-400">task</span> construct nested within a <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> directive, using <span className="font-semibold text-indigo-600 dark:text-indigo-400">firstprivate</span> and <span className="font-semibold text-indigo-600 dark:text-indigo-400">shared</span> clauses to manage variable scoping:
        </p>
        <details className="mt-4">
          <summary className="cursor-pointer text-lg font-medium text-indigo-600 dark:text-indigo-400 hover:underline">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed mt-4">
          The ASR representation below showcases the nested structure, with the <span className="font-semibold text-indigo-600 dark:text-indigo-400">task</span> construct properly embedded within a <span className="font-semibold text-indigo-600 dark:text-indigo-400">section</span> region:
        </p>
        <details className="mt-4">
          <summary className="cursor-pointer text-lg font-medium text-indigo-600 dark:text-indigo-400 hover:underline">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300 leading-relaxed mt-4">
          The third MRE, also focusing on the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct, follows a similar structure and is included in PR <a href="https://github.com/lfortran/lfortran/pull/7449" className="text-indigo-500 dark:text-indigo-400 hover:underline">#7449</a>. These MREs validate the stack-based approach’s ability to handle nested OpenMP directives, ensuring that LFortran can accurately represent complex parallel constructs in the ASR.
        </p>
      </div>
      ...
      <div>
        ...
        <ul className="list-disc pl-6 mt-4 space-y-2 text-lg text-gray-700 dark:text-gray-300">
          <li>Extend the OpenMP pass to visit the <span className="font-semibold text-indigo-600 dark:text-indigo-400">OMPRegion</span> node and support the <span className="font-semibold text-indigo-600 dark:text-indigo-400">sections</span> construct, lowering it to appropriate runtime calls.</li>
